...
Оба профиля поставляются с `fsync: true` и уже указывают друг друга в списке `peers`, поэтому для локального стенда править конфигурацию вручную не нужно.

Если оба процесса запускаются на одной машине, порт Actuator у второго нужно переопределить, иначе он не стартует:
