...
### Проверка и типовые сбои

| Проверка | Ожидание |
|----------|----------|
| Обе readiness UP | SQL слушает; при репликации — ORCHID синхронизирован |
| Один `writerEligible: true` | Обычно primary; мета клиента совпадает |
| Smoke-запись на **15432**, чтение через `readEndpoints=…:15433` | Строка видна после подтягивания (`applyLagStale` false) |

| Симптом | Вероятная причина |
|---------|-------------------|
| `bad frameLen …` у клиента | Клиент попал на порт **репликации** (5615/5616), не SQL |
| Второй процесс на том же `dataDir` | Два писателя / порванные файлы — остановить лишний |
| Readiness DOWN без конца | Пиры недоступны или ORCHID не sync — смотреть `peers` и детали Actuator |
| AUTH fail на реплике после `CREATE USER` только на primary | `privileges.meta` на каждом узле — [безопасность](../configure-and-operate/operations/security.md) |
