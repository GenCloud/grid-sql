...
- **Только собственный протокол приложений.** Сервисы работают по `grid://` или `jdbc:grid://` (кадры Grid little-endian).
- **Не работает с общим каталогом `dataDir`** на NFS/SAN для всего кластера: каталог принадлежит одному узлу.
- **Не допускает двух пишущих узлов одновременно.** Пишущий один; роль меняется через `PROMOTE_NOTIFY` / `rediscoverWriter()`, а не перебором хостов в URL.
- **Не требует, чтобы весь набор данных помещался в память.** В памяти — рабочий набор; при долговременном хранении источник истины — запечатанные файлы и журнал.
- **Не обещает TLS на SQL-порту.** Терминация TLS — перед узлом (прокси / балансировщик); см. [безопасность](../configure-and-operate/operations/security.md).
