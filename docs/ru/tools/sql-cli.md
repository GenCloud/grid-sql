...
URL записи HA (несколько хостов в authority) — как у приложений:

```
grid://user:pass@127.0.0.1:15432,127.0.0.1:15433/public
```

Дожидайтесь readiness Actuator UP перед ожиданием успешного connect (`/health/readiness` на **7777** primary / **7778** replica или capacity). После смены роли откройте новую сессию CLI или переподключитесь — интерактивная оболочка сама `rediscoverWriter()` не вызывает.
