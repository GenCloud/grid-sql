...
- **Числовые литералы со знаком.** Ведущий минус входит в грамматику (`'-'? INT` / decimal); bind может передать отрицательный INT/LONG. Порядок сравнения фиксированной ширины INT/LONG в индексе и в остаточном фильтре — знаковый (`Integer.compare` / `Long.compare`).
...
| `TRUNCATE TABLE` | Полная очистка таблицы | Тот же путь журнала / TX, что у DELETE: при сбое — отказ клиенту, без частичной видимости |
