...
# PASS by p50 alone is forbidden.
param(
...
# Ref B (v11 best nochao absolute targets before tolerance)
$ref = @{
  write_p50 = 12.7
  write_p95 = 88.1
  write_p99 = 252.4
  ...
  append_p95 = 36.8
  append_p99 = 47.4
  txn_r_p95 = 25.0   # plan target (hard)
  txn_r_p99 = 55.0   # soft vs ~47-scale
}
...
function Add-Check([string]$Name, [Nullable[double]]$Got, [double]$Base, [string]$Kind) {
  ...
  if (-not $ok -and $Kind -eq "hard") { $script:hardFail = $true }
...
# Hard gates (p50+p95) — Ref B * (1+Tol); txn_r p95 uses absolute plan target 25
...
Write-Output "=== qg-gate Ref B (tol p50/p95=+$([int]($Tol*100))% p99=+$([int]($P99Tol*100))%) ==="
...
# Forbid claiming PASS if only p50 would pass — already enforced by requiring p95 checks above.
